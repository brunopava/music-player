...
public class bl_AudioPlayer : MonoBehaviour {
    ...
    {
        if (tracks.Count > 1 && !m_Repeat)
        {
            if (forward) { 
                CurrentClip++;
                if(CurrentClip >= tracks.Count)
                {
                    CurrentClip = 0;
                }
            }
            ...
            { 
                ...
                if(CurrentClip < 0)
                {
                    CurrentClip = tracks.Count - 1;
                } 
            }

            // NewClip(tracks[CurrentClip].clip);
            Debug.Log("TODO:");
            PlayPause();
            StopAllCoroutines();
    ...
    public void PlayPause()
    {
        ...
        {
            Debug.Log("TODO");
            // if (tracks != null) { NewClip(tracks[0].clip); } else { Debug.Log("Dont have clip yet!"); return; } 
        }
